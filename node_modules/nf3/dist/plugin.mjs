import { DEFAULT_CONDITIONS, guessSubpath, pathRegExp, toImport, toPathRegExp } from "./_chunks/trace.mjs";
import { isAbsolute, resolve } from "./_chunks/libs/pathe.mjs";
import { resolveModulePath } from "./_chunks/libs/exsolve.mjs";
...
function externals(opts) {
	...
	return {
		...
		writeBundle: {
			order: "post",
			async handler() {
				if (opts.trace === false || tracedPaths.size === 0 && !opts.traceInclude?.length) return;
				...
				const { traceNodeModules } = await import("./_chunks/trace.mjs").then((n) => n.trace_exports);
